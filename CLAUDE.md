...
4. **正規フローを必ず通してから push する** — コメント対応で変更が発生した場合、修正が軽微でも以下のスキルを省略しないこと
   ...
   > **重要**: push 前に上記スキルをすべて実行し、全指摘を解消してから push すること。「軽微だから」「急ぐから」という理由でスキップしてはならない。ユーザーに「正規のフローを通ってください」と言われた場合もこのルールが適用される。
...
**モックアップ画像の参照先: `specs/images/` が確定版。`docs/02_requirements/mockups/` は旧版のため参照しないこと。**
