...
7. **AIPO 仕様を確認せずに設計・実装しない。** 要件定義書に記載がない事柄は、必ず AIPO ソースコード（`gh api repos/arkjun/aipo/...`）または実 DB を調査してから仕様を決める。独自判断で「こうだろう」と実装してはならない。逸脱する場合はユーザーに確認を取ること。
8. **GitHub MCP は使わない。`gh` コマンドを使う。**
