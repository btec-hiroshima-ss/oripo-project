...
- ページリロード後は自分のみに戻る（選択状態は localStorage 等に保存しない）
...
- グループ一覧（turbine_group の group_alias_name が存在するグループ、システムグループ除外）をタブまたはリストで表示する
- グループを展開するとメンバー一覧（アクティブユーザーのみ）を表示する
- 氏名フリーワード検索でユーザーを絞り込める
- チェックで追加・解除できる。既に選択済みのユーザーは checked 状態で表示
- 自分自身は常に selected で表示し選択解除できない
...
- 「参加ユーザー選択」ボタンをクリックするとユーザーピッカーモーダルが開く
  - マルチユーザービューと同じモーダルコンポーネントを再利用する（`UserPickerModal`）
  - ただし「自分自身は選択解除不可」の制約はなく、作成者以外の参加者を管理するものとする
