...
- タブの x（削除）ボタンはアクティブなタブにのみ表示する
- タブ追加（+）ボタンはヘッダー右端に固定表示し、個人設定表示中も常に表示する（AIPO 準拠）
- 個人設定はヘッダーの「個人設定」ボタンでコンテンツエリアを切り替える方式（URL は `/` のまま）。
  別 URL にしないことでページタブを全画面で常に表示できる（AIPO から逸脱、UI 簡素化のため）
