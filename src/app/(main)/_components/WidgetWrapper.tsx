...
export default function WidgetWrapper({ widgetId, widgetType, children, onDeleted }: Props) {
  ...
  // このウィジェットがドロップ先として選ばれているとき（= ここに挿入される）に挿入線を表示する
  const { active, over } = useDndContext()
  const isInsertTarget = over?.id === widgetId && active?.id !== widgetId
  ...
  return (
    <div>
      {isInsertTarget && (
        <div className="h-0.5 bg-brand rounded-full mb-2 -mt-1" />
      )}
      <div
        ref={setNodeRef}
        style={style}
