...
#### 💡 プライバシー方針
近年のプライバシーポリシーに配慮し、業務上必要な個人情報のみ保持する設計とする。メールアドレス等、Oripo では保持しない項目がある。電話番号（内線・外線）は業務上必要なため保持する。携帯電話番号は保持しない。
...
- ユーザーの新規作成・編集・削除
  - 登録項目: ログインID（変更不可）、パスワード、姓・名、フリガナ（姓・名）、部署（複数選択可）、役職、内線番号、電話番号、管理者権限フラグ
...
**要件:**
- ユーザー情報
  - 自分のプロフィール情報（ログイン名、氏名、フリガナ、部署、役職、権限）を閲覧できる
...
- 電話番号（内線・外線）は業務上必要なため移行する（携帯電話番号は移行しない）
