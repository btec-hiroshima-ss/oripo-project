...
#### 💡 プライバシー方針
近年のプライバシーポリシーに配慮し、業務上必要な個人情報のみ保持する設計とする。メールアドレス等、Oripo では保持しない項目がある。電話番号は携帯電話番号（社内）として保持する。
...
![スケジュール（予定詳細）](../../specs/images/スケジュール詳細.png)
...
- 表示項目: 氏名、部署、グループ、携帯電話番号（社内）
- 閲覧専用（編集はユーザー情報管理で行う）
- 閲覧専用（電話番号からの直接発信は今後の機能）
...
- ユーザーの新規作成・編集・削除
  - 登録項目: ログインID（変更不可）、パスワード、姓・名、フリガナ（姓・名）、部署（複数選択可）、携帯電話番号（社内）、管理者権限フラグ
...
![ユーザー管理](../../specs/images/ユーザー管理.png)

![部署管理](../../specs/images/部署管理.png)
...
**要件:**
- ユーザー情報
  - 自分のプロフィール情報（ログイン名、氏名、フリガナ、部署、権限）を閲覧できる
  - 「ユーザー情報とパスワードを編集する」ボタンから自分の情報を編集できる（編集項目は 2.6 の登録項目に準じる）
- ページ設定（タブ管理）
  - マイページを複数作成・編集・削除・並び順変更できる
  - デフォルトで「マイページ」が1つ存在する
- マイグループ管理
  - 任意のユーザーと設備をまとめたグループを作成・編集・削除できる
  - スケジュール（全員スケジュール）・ユーザー名簿の絞り込みに利用できる
  - 自分だけが参照できる（他ユーザーには非公開）

**モックアップ:**

![個人設定ユーザー情報](../../specs/images/個人設定ユーザー情報.png)

![Myグループ追加](../../specs/images/Myグループ追加.png)

![ページ設定ページ追加](../../specs/images/ページ設定ページ追加.png)
...
- 電話番号（内線・外線・携帯）は全て移行する（Oripo では携帯電話番号（社内）として管理）
- 古いスケジュールレコード（３年以上前）は移行しない方針
- パスワードはそのままコピー（ユーザーへの通知・変更不要）
- **ホーム画面のページ設定（タブ構成・ウィジェット配置）は移行しない。** AIPO のページ設定は Jetspeed フレームワーク固有のバイナリ形式（PSML）で保存されており移行が困難なため、Oripo への切り替え時に初期化される
