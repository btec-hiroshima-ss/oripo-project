...
> ⚡ **パフォーマンス**: 現行 AIPO の応答遅延・高負荷の課題を解消し、業務利用に十分な応答性能を確保する。

> 🔒 **セキュリティ**: 外部公開に十分なセキュリティ水準を確保する。認証・通信・アクセス制御を適切に設計する。

> 🎨 **モダンなデザイン**: 現行 AIPO から刷新し、使いやすく現代的な UI を提供する。

> 💡 **プライバシー方針**: 近年のプライバシーポリシーに配慮し、業務上必要最低限の個人情報のみ保持する設計とする。AIPO にある個人情報項目（電話番号・メールアドレス等）でも、Oripo では保持しないものがある。

> 🔧 **拡張性**: 今後の機能追加（AI エージェント連携等）を前提とした設計とする。

> 📚 **スキルアップ**: 新入社員・待機社員のスキルアップ用教材としての活用も目的の一つ。改良・保守を通じて技術習得ができる構成とする。
...
> ⚠️ **プライバシー方針に基づき、AIPO より保持情報を最小化。**
...
> ⚠️ **プライバシー方針に基づき、業務上必要最低限の情報のみ管理。**
...
- ユーザーの新規作成・編集・削除
  - 登録項目: ログインID（変更不可）、パスワード、姓・名、部署（複数選択可）、役職、管理者権限フラグ
  - パスワードは AIPO と同アルゴリズムで保存。移行時にそのままコピーするためパスワード変更不要
- 部署マスタ管理（追加・編集・削除・並び順変更）
- 役職マスタ管理（追加・編集・削除）
- 設備マスタ管理（追加・編集・削除、設備グループ管理）
- 管理者権限を持つユーザーのみ操作可能
...
- 想定スペック（目安）:
  - メモリ: 4GB 以上（現在の同時利用規模では 2GB で動作するが、機能拡張を見据えて 4GB 推奨）
...
- 新入社員・待機社員のスキルアップ教材として、改良・保守を通じた技術習得を促進する構成とする
