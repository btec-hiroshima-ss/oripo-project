...
#### 💡 プライバシー方針
近年のプライバシーポリシーに配慮し、業務上必要な個人情報のみ保持する設計とする。メールアドレス等、Oripo では保持しない項目がある。携帯付電話番号（社内用）のみ保持する。内線番号・外線番号・携帯電話番号は保持しない。
...
- ユーザーの新規作成・編集・削除
  - 登録項目: ログインID（変更不可）、パスワード、姓・名、フリガナ（姓・名）、部署（複数選択可）、携帯付電話番号（社内用）、管理者権限フラグ
...
- 携帯付電話番号（社内用）のみ移行する（内線番号・外線番号・携帯電話番号は移行しない）
